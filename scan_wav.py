...
model.load_state_dict(torch.load(os.path.join(config.models_dir + '/Test', 'weights_50.pt'), map_location=device)['model_state_dict'])
...
class Scanner():
    ...
    def read_audio_section(self, wav_path, start_time, end_time, sr):
        
        wav, fs = librosa.load(wav_path, offset=start_time, duration=(end_time - start_time), sr=sr)

        return fs, wav

    def scan_wav(self, new_wav_path, save_path, fs_target, T_target, batch_size, wav_fs, chunk_mins=10):
        
        # get duration of wav file in seconds
        wav_duration = librosa.get_duration(filename=new_wav_path, sr=wav_fs)
        
        # number of 10 minute chunks to process (ignore last one for simplicity)
        num_chunks = math.floor(wav_duration / 60 / chunk_mins)
        chunk_secs = chunk_mins*60 # seconds in 10 minutes
        ...
        for _ in range(num_chunks):
            ...
            # get chunk
            fs0, s0 = self.read_audio_section(new_wav_path, curr_sample / wav_fs, (curr_sample / wav_fs) + chunk_secs, wav_fs)
            ...
            for batch in tqdm(range(n_batches), disable= not args.verbose):
                ...
                for i in range(batch_size):
                    ...
                    # Get a window
                    s_curr = s0[curr_chunk_sample:curr_chunk_sample+n_samples]
                    ...
                    # Iterate samples
                    curr_chunk_sample += n_samples

            # Calculate spectrograms
            X = dataLoader.to_spect(S).squeeze(axis=1).copy()

            # Predict current batch
            X_torch = transforms(torch.from_numpy(X).float()).to(device)
            ...
            if len(S) > 0:
                print(f"saving {len(S)} detections")
            ...
            curr_sample += chunk_secs*fs0
            print(curr_sample / wav_fs)
if __name__== "__main__":
    ...
    for file_idx in range(len(wav_files)):
        scanner.scan_wav(os.path.join(args.acoustic_data, wav_files[file_idx]), args.save_dir, fs_target, T_target, args.batch_size, wav_fs)